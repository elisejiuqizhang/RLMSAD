...
def testing_scores(model, test_window, alpha=.5, beta=.5): # Compute the score for a single window
    test_window=to_device(test_window,device)
    w1=model.decoder1(model.encoder(test_window))
    w2=model.decoder2(model.encoder(w1))
    result=alpha*torch.mean((test_window-w1)**2,dim=1)+beta*torch.mean((test_window-w2)**2,dim=1)
    return result
